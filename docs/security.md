...
Web 视频手控还须满足 [RFC 0008](rfc/0008-web-control-safety-and-recovery.md)：实际采集到
显示的画面年龄不得超过 500 ms，旧帧或未知年龄不能取得/续用控制；视频恢复不自动恢复
运动。最后观看者离开后采集自动结束，不能仅靠长期在线的 Bridge 心跳维持无人观看的预览。

固件升级须在可恢复布局和可信镜像验证成立时执行。本地安全自检成功可保留新版本，
主机离线只使 rollout 等待确认；不能因等待主机而无限延迟本地启动确认。设备恢复出厂
只删除设备用户配置与配对，不删除主机会话、长期记忆或审计，也不清除硬安全常量和恢复信任。
